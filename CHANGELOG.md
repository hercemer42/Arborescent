...
- **Resumable AI sessions per workflow step**: Each workflow step captures its AI session id on the node and persists it across app restart. Right-click a workflow node → **Resume session** focuses the original terminal tab if still open, or opens a new one in the recorded working directory and runs `claude --resume`. Closing a terminal tab no longer ends the underlying session — you can walk away from a running step and come back to it from any tab. A red indicator shows when a session is gone from Claude's session store and can't be resumed.
