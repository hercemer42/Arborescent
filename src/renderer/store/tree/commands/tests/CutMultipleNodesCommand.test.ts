...
describe('CutMultipleNodesCommand', () => {
  ...
  it('should delete multiple nodes on execute', () => {
    const command = new CutMultipleNodesCommand(
      ['node-1', 'node-3'],
      getState,
      setState,
  ...
  it('should have correct description', () => {
    ...
    expect(command.description).toBe('Cut 3 node(s)');
  });

  it('should return cut node IDs', () => {
    const command = new CutMultipleNodesCommand(
      ['node-1', 'node-3'],
      getState,
      setState,
    ...
    expect(command.getCutNodeIds()).toEqual(['node-1', 'node-3']);
  });
