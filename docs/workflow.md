...
When a terminal or MCP review delivers its revision, Arborescent opens a zoom tab focused on that branch in the background and flags it in the tab bar, so a pending review is easy to find in a large tree without pulling you off what you're doing. Click the flagged tab to open the review in its own view — the flag clears once you do. The zoom tab shows the proposed subtree fully expanded; inline on the main tree a review follows the branch's own collapsed state, so reviewing a branch you had collapsed keeps your outline compact while you decide. Accept and Cancel work there exactly as on the main tree; resolving from the zoom tab closes it and returns you to the main tree.
...
Panel state persists across restarts. When you reopen Arborescent, each file's panel visibility, browser tabs, and terminals are restored — for every open file, not just the one in view. Terminal history is lost (processes can't survive a restart), but each terminal reopens in its working directory, and any terminal that was running a Claude session resumes it automatically. Resuming runs in the background, so the window is interactive immediately; a terminal whose session or folder is no longer available reopens as a plain shell.
